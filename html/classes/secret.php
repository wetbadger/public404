...
<!--
window.open ('preview.php', 'newwindow', config='height=400, width=400, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, directories=no, status=no')
-->
</SCRIPT>
...
$stmt->bindParam(':page' , $page);
$stmt->bindParam(':ipaddress' , preg_replace('/[.]/', '', $ipaddress));
$stmt->execute();
$ts = $stmt->fetchColumn();
$ts = strtotime($ts);
...
//fill table with row of data when user presses submit
 ...
 //because I hate arrays...
	...
	$stmt->bindParam(':ip', preg_replace('/[.]/', '', $ipaddress));
	$stmt->execute();
       if ($page == "meta") {
		   echo "Your post was submitted to <a href='http://www.thepublic404.com/meta'>meta</a>"; // !!!!! SERVER SENSITIVE !!!!!
	   } else {
