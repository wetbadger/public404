...
<table border="0" cellpadding="1" cellspacing="1">
  <tr>
    ...
    <td width="79" align="center" scope="col"><a href="donate">Donate</a></td>
  </tr>
</table>
       <hr noshade size=3>
  <!-- WELCOME MESSAGE -->
...
<h2>secret.php</h2>
<p>Congratulations, you have found the secret page. From this box you may submit content to <a href="http://www.public404.com">the website: </a><!--SERVER SENSITIVE-->
  <!-- HTML FORM -->
</p>
...
if (isset($_POST['page']) && isset($_POST['submit'])) { //unique pages, whether new or old, can get rewritten each time someone submits one. this should be in place if new index rules have been imposed
...
$creation->create_new_page($page, $qpage, $q, NULL);
...
//check each page owned by the mod
				...
				if ($page1 == '') {
					...
					echo $page . " was added your mod list. You cannot mod any more pages. <br />";
										 	...
										 	if ($_SESSION['status'] == 'authorized' and $page != '') {
			echo "You cannot mod " . $page . "<br />"; //if for some other reason the page can't be modded. (owned by someone else or too many pages)
														  } //if not logged in don't echo anything else
...
$stmt->bindParam(':page' , $page);
$stmt->bindParam(':ipaddress' , $shortip);
$stmt->execute();
$ts = $stmt->fetchColumn();
$ts = strtotime($ts);
if (!empty($ts) and $ts > (time() - 60)) { //if latest post by ip on page was less than 1 minutes ago
if (!isset($_SESSION['state'])) {
			@session_start();
			$_SESSION['state'] = 'preview';
			$_SESSION['preview'] = $_POST['post'];
			...
			//always
			$_SESSION['i'] = 'pass';
			$_SESSION['wait'] = "You must wait " . ($ts - (time() - 60)) . " seconds before posting on this page again.";
			echo '<meta http-equiv="refresh" content="0;URL=\'http://www.public404.com/secret.php\'">';
	} else {
...
//fill table with row of data when user presses submit
 ...
 //because I hate arrays...
	...
	$stmt->execute();
       if ($page == "") {
		   echo "Your post was submitted to <a href='http://www.public404.com'>the main page.</a>"; // !!!!! SERVER SENSITIVE !!!!!
	   } else {
     echo "Your post was submitted to <a href='" . $page . "'>" . $page . "</a> ";
	 }
} catch(PDOException $e) {
  echo 'Error: ' . $e->getMessage();
}//end catch
...
//insert from include standardtags.php
