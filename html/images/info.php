...
<title>images/info.php</title>
</head>

<body>

<center>Info for <a href="http://www.public404.com/images">images</a></center>

<br />
    <center>State: <?php  //determine if page is frozen
    $stmt = $conn->prepare("SELECT * FROM freeze WHERE page = 'images'");
	$stmt->execute();
...
?></div></td>
	...
	$stmt = $conn->prepare("SELECT * FROM mods WHERE page1 = 'images' OR page2 = 'images' OR page3 = 'images'");
	$stmt->execute();
	$mod = $stmt->fetchAll();
	foreach ($mod as $var) {
		echo $var["username"] . "
...
$stmt = $conn->prepare("SELECT * FROM bstr WHERE PAGE = 'images'");
$stmt->execute();
...
?></div></td>
    <td><div name="xtags4" id="xtags4" style="overflow:auto; width:300px; height:185px; border-style:solid; border-width:thin; border-color:#999; white-space:pre; font-family:monospace; font-weight:normal;" align="left"><?php //get str and rpl
$stmt = $conn->prepare("SELECT * FROM bregex WHERE PAGE = 'images'");
$stmt->execute();
$strrpl = $stmt->fetchAll();
echo "<table width='300'>";
foreach ($strrpl as $var) {
...
$stmt = $conn->prepare("SELECT * FROM history WHERE PAGE = 'images' ORDER BY `history`.`id` DESC");
$stmt->execute();
$history = $stmt->fetchAll();

foreach ($history as $var) {
echo "<p><a href='http://www.public404.com/images/" . $var['id'] . ".html'>" . $var['timestamp'] . "</a></p>";
}
?> </div>
</body>
</html>