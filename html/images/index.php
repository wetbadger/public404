<?php
		...
		//$_SERVER["DOCUMENT_ROOT"] = "/var/chroot/home/content/41/8949141/html";
		    //include $_SERVER["DOCUMENT_ROOT"] . "/classes/connect.php";
				$path = $_SERVER["DOCUMENT_ROOT"];
					 ...
					 $stmt = $conn->prepare("SELECT state FROM freeze WHERE page = 'images'"); //add this code to allow pages to be banned
 				 $stmt->execute();
 ...
 $stmt = $conn->prepare("SELECT COUNT( post ) FROM posts WHERE page = 'images' AND post != ''");
	 $stmt->execute();
...
//get allowed tags
$stmt = $conn->prepare("SELECT tag FROM btag WHERE PAGE = 'images'");
$stmt->execute();
$tag = $stmt->fetchColumn();

//get strings
$stmt = $conn->prepare("SELECT str FROM bstr WHERE PAGE = 'images'");
$stmt->execute();
$strSch = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
$stmt = $conn->prepare("SELECT rpl FROM bstr WHERE PAGE = 'images'");
$stmt->execute();
$strRpl = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);

//get regexes
$stmt = $conn->prepare("SELECT regex FROM bregex WHERE PAGE = 'images'");
$stmt->execute();
$regexSch = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
$stmt = $conn->prepare("SELECT rpl FROM bregex WHERE PAGE = 'images'");
$stmt->execute();
$regexRpl = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);

// display data
try {
$stmt = $conn->prepare("SELECT * FROM posts WHERE PAGE = 'images' AND post != '' AND timestamp >= (select `posts`.`timestamp` from posts WHERE PAGE = 'images' AND post != '' ORDER by timestamp DESC LIMIT 1 OFFSET :offset) ORDER BY `posts`.`uid` ASC");
