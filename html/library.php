...
<head>
<title>library.php</title><!-- HTML HEADER --></head>
...
<table border="0" cellpadding="1" cellspacing="1">
  <tr>
    ...
    <td width="79" align="center" scope="col"><?php if (!isset($_SESSION['status']) or $_SESSION['status'] != 'authorized') { echo'<a href="login.php">Log in</a>'; } else { echo '<a href="mod.php">Mod</a>'; }?></td>
  </tr>
</table>
...
try {
    $stmt = $conn->prepare('SELECT DISTINCT page FROM posts WHERE page != "" ORDER BY page ASC');
    $stmt->execute();
	$num = $stmt->rowCount();
	print_r('There are ' . $num . ' pages on <a href="http://public404.com">public404</a><br>');
...
$sub = 'hide';
if (isset($_GET['id'])) {
	echo '<a href="?id=' . $_GET['id'] . '&sort=' . $_GET['sort'] . '&sub=show">Show subpages</a><br /><br />';
} else {
echo '<a href="?sub=show">Show subpages</a><br /><br />'; }
} elseif($_GET['sub'] == 'show') {
$sub = 'show';
if (isset($_GET['id'])) {
	echo '<a href="?id=' . $_GET['id'] . '&sort=' . $_GET['sort'] . '&sub=hide">Hide subpages</a><br /><br />';
} else {
...
try {
	if ($sort == 'alphabet') {
    $stmt = $conn->prepare('SELECT DISTINCT page FROM posts WHERE page != "" AND page > :name ORDER BY page ASC');
	$stmt->bindParam(':name', $name);
	$stmt->execute();
	$page = $stmt->fetchAll();
	foreach ($page as $var) {
		if (strpos($var['page'],"/") === false || $_GET['sub'] == 'show') {
	echo('<a href="' . $var['page'] . '/">' . $var['page'] . '</a> <a href="' . $var['page'] . '/info.php">*</a><br />');//page break to show pages in list forma
		}
	}//end loop
	} elseif ($sort == 'tebahpla') {
    $stmt = $conn->prepare('SELECT DISTINCT page FROM posts WHERE page != "" AND page > :name ORDER BY page DESC');
	$stmt->bindParam(':name', $name);
	$stmt->execute();
	$page = $stmt->fetchAll();
	foreach ($page as $var) {
	...
	$stmt = $conn->prepare('SELECT DISTINCT PAGE FROM `posts` WHERE page != "" ORDER BY `posts`.`timestamp` DESC');
	$stmt->execute();
	$new = $stmt->fetchAll();

	foreach ($new as $var) {
		if (strpos($var['PAGE'],"/") === false || $_GET['sub'] == 'show') {
	echo '<a href="' . $var['PAGE'] . '/">' . $var['PAGE'] . '</a> <a href="' . $var['PAGE'] . '/info.php">*</a><br />';
		}
	}

	} else if ($sort == 'old') {

	$stmt = $conn->prepare('SELECT DISTINCT PAGE FROM `posts` WHERE page != "" ORDER BY `posts`.`timestamp` ASC');
	$stmt->execute();
	$old = $stmt->fetchAll();
	...
	$stmt = $conn->prepare('SELECT DISTINCT page, count( DISTINCT ipv4 ) AS frequency FROM posts WHERE page != "" GROUP BY page ORDER BY count( DISTINCT ipv4 ) DESC');
	$stmt->execute();
	$new = $stmt->fetchAll();
	...
	$stmt = $conn->prepare('SELECT DISTINCT page, count( DISTINCT ipv4 ) AS frequency FROM posts WHERE page != "" GROUP BY page ORDER BY count( DISTINCT ipv4 ) ASC');
	$stmt->execute();
	$new = $stmt->fetchAll();
