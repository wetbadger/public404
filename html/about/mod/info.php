...
<title>about/mod/info.php</title>
</head>

<body>

<center>Info for <a href="http://www.public404.com/about/mod">about/mod</a></center>

<br />
    <center>State: <?php  //determine if page is frozen
    $stmt = $conn->prepare("SELECT * FROM freeze WHERE page = 'about/mod'");
	$stmt->execute();
...
?></div></td>
	...
	$stmt = $conn->prepare("SELECT * FROM mods WHERE page1 = 'about/mod' OR page2 = 'about/mod' OR page3 = 'about/mod'");
	$stmt->execute();
	$mod = $stmt->fetchAll();
	foreach ($mod as $var) {
		echo $var["username"] . "
...
$stmt = $conn->prepare("SELECT * FROM bstr WHERE PAGE = 'about/mod'");
$stmt->execute();
...
?></div></td>
    <td><div name="xtags4" id="xtags4" style="overflow:auto; width:300px; height:185px; border-style:solid; border-width:thin; border-color:#999; white-space:pre; font-family:monospace; font-weight:normal;" align="left"><?php //get str and rpl
$stmt = $conn->prepare("SELECT * FROM bregex WHERE PAGE = 'about/mod'");
$stmt->execute();
$strrpl = $stmt->fetchAll();
echo "<table width='300'>";
foreach ($strrpl as $var) {
...
$stmt = $conn->prepare("SELECT * FROM history WHERE PAGE = 'about/mod' ORDER BY `history`.`id` DESC");
$stmt->execute();
$history = $stmt->fetchAll();

foreach ($history as $var) {
echo "<p><a href='http://www.public404.com/about/mod/" . $var['id'] . ".html'>" . $var['timestamp'] . "</a></p>";
}
?> </div>
</body>
</html>