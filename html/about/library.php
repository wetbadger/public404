...
<head>
<title>about/library.php</title><!-- HTML HEADER --></head>
...
try {
    $stmt = $conn->prepare('SELECT DISTINCT page FROM posts WHERE page LIKE "about/%" ORDER BY page ASC');
    $stmt->execute();
	$num = $stmt->rowCount();
	print_r('There are ' . $num . ' pages in about<br />');
...
try {
	if ($sort == 'alphabet') {
    $stmt = $conn->prepare('SELECT DISTINCT page FROM posts WHERE page LIKE "about/%" AND page > :name ORDER BY page ASC');
	$stmt->bindParam(':name', $name);
	$stmt->execute();
	$page = $stmt->fetchAll();
	foreach ($page as $var) {
		if (strpos(preg_replace("#about/#", "", $var['page'], 1),"/") === false || $_GET['sub'] == 'show') {
	echo('<a href="' . preg_replace("#about/#", "", $var['page'], 1) . '/">' . preg_replace("#about/#", "", $var['page'], 1) . '</a> <a href="' . preg_replace("#about/#", "", $var['page'], 1) . '/info.php">*</a><br />');//page break to show pages in list forma
		}
	}//end loop
	} elseif ($sort == 'tebahpla') {
    $stmt = $conn->prepare('SELECT DISTINCT page FROM posts WHERE page LIKE "about/%" AND page > :name ORDER BY page DESC');
	$stmt->bindParam(':name', $name);
	$stmt->execute();
	$page = $stmt->fetchAll();
	foreach ($page as $var) {
	...
	$stmt = $conn->prepare('SELECT PAGE FROM `posts` WHERE page LIKE "about/%" ORDER BY `posts`.`timestamp` DESC');
	$stmt->execute();
	$new = $stmt->fetchAll();
	...
	$stmt = $conn->prepare('SELECT PAGE FROM `posts` WHERE page LIKE "about/%" ORDER BY `posts`.`timestamp` ASC');
	$stmt->execute();
	$old = $stmt->fetchAll();
	...
	$stmt = $conn->prepare('SELECT DISTINCT PAGE FROM `posts` WHERE page LIKE "about/%" ORDER BY `posts`.`views` DESC');
	$stmt->execute();
	$new = $stmt->fetchAll();
	...
	$stmt = $conn->prepare('SELECT DISTINCT PAGE FROM `posts` WHERE page LIKE "about/%" ORDER BY `posts`.`views` ASC');
	$stmt->execute();
	$new = $stmt->fetchAll();
