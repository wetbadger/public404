<?php
		...
		//$_SERVER["DOCUMENT_ROOT"] = "/var/chroot/home/content/41/8949141/html";
		    //include $_SERVER["DOCUMENT_ROOT"] . "/classes/connect.php";
				$path = $_SERVER["DOCUMENT_ROOT"];
					 ...
					 $stmt = $conn->prepare("SELECT state FROM freeze WHERE page = 'donate'"); //add this code to allow pages to be banned
 				 $stmt->execute();
 ...
 $stmt = $conn->prepare("SELECT COUNT( post ) FROM posts WHERE page = 'donate' AND post != ''");
	 $stmt->execute();
...
//get strings
$stmt = $conn->prepare("SELECT str FROM bstr WHERE PAGE = 'donate'");
$stmt->execute();
$strSch = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
$stmt = $conn->prepare("SELECT rpl FROM bstr WHERE PAGE = 'donate'");
$stmt->execute();
$strRpl = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);

//get regexes
$stmt = $conn->prepare("SELECT regex FROM bregex WHERE PAGE = 'donate'");
$stmt->execute();
$regexSch = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
$stmt = $conn->prepare("SELECT rpl FROM bregex WHERE PAGE = 'donate'");
$stmt->execute();
$regexRpl = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);

// display data
try {
$stmt = $conn->prepare("SELECT * FROM posts WHERE PAGE = 'donate' AND post != '' AND timestamp >= (select `posts`.`timestamp` from posts WHERE PAGE = 'donate' AND post != '' ORDER by timestamp DESC LIMIT 1 OFFSET :offset) ORDER BY `posts`.`uid` ASC");
